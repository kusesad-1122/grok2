...
class ChatAdapter(private val items: MutableList<ChatItem>) :
    ...
    override fun onBindViewHolder(holder: VH, position: Int) {
        ...
        when (val item = items[position]) {
            is ChatItem.User -> {
                holder.role.text = ctx.getString(R.string.role_user)
            ...
            is ChatItem.Assistant -> {
                holder.role.text = ctx.getString(R.string.role_assistant)
            ...
            is ChatItem.ToolCall -> {
                holder.role.text = "▶ ${ctx.getString(R.string.role_tool_call)}:${item.name}"
            ...
            is ChatItem.ToolResult -> {
                holder.role.text = "◀ ${ctx.getString(R.string.role_tool_result)}:${item.name}"
            ...
            is ChatItem.System -> {
                holder.role.text = ctx.getString(R.string.role_system)
            ...
            is ChatItem.Error -> {
                holder.role.text = ctx.getString(R.string.role_error)
