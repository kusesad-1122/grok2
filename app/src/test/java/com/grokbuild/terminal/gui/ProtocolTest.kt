...
class ProtocolTest {
    ...
    fun openai_request_body_shape() {
        val body = OpenAiClient.buildRequestBody("gpt-4o", "系统", listOf(ChatMessage("user", "你好")), tools)
    ...
    fun anthropic_request_body_shape() {
        val body = AnthropicClient.buildRequestBody("claude-3-5-sonnet-latest", 4096, "系统", listOf(ChatMessage("user", "你好")), tools)
        ...
        assertEquals("run_shell", body.getJSONArray("tools").getJSONObject(0).getString("name"))
        assertTrue(body.getJSONArray("tools").getJSONObject(0).has("input_schema"))
    }
